...
if [[ "$INSTALL_K3S" -eq "true" ]]; then
    curl -sfL https://get.k3s.io | K3S_KUBECONFIG_MODE='644' INSTALL_K3S_EXEC='--disable traefik --disable servicelb --tls-san $(curl http://169.254.169.254/latest/meta-data/local-ipv4)' sh -s -
    ...
    sed -i "s/127.0.0.1/$(curl http://169.254.169.254/latest/meta-data/local-ipv4)/" /home/${USERNAME}/.kube/public_ip_config
