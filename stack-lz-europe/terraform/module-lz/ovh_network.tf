...
resource "ovh_cloud_project_network_private" "private_network" {
  ...
  name          = "${var.cy_org}-${var.cy_pro}-${var.cy_env}" # Network name
  regions       = [var.ovh_regions[each.key]]
  vlan_id       = each.key # vRack vlan ID

  depends_on    = [ovh_vrack_cloudproject.vrack] # Depends on vRack being associated with the cloud project
}
...
resource "ovh_cloud_project_network_private_subnet" "private_subnets" {
 ...
 resource "ovh_vrack_cloudproject" "vrack" {
  service_name = var.ovh_project_id
  project_id   = var.ovh_tenant_id
} 